...
func ParsePKIXPublicKey(derBytes []byte) (pub interface{}, err error) {
	...
	return parsePublicKey(algo, &pki, nil)
}
...
func parsePublicKey(algo PublicKeyAlgorithm, keyData *publicKeyInfo, needsReencode *bool) (interface{}, error) {
	asn1Data := keyData.PublicKey.RightAlign()
	switch algo {
	case RSA:
		// CHADRON: Android does not care, ignore.
		// Sample: fcfbe912bc70e9c996c82cc25194908055734f7db4e81fd948fb7deb8bc7450f
		//
		...
		if p.N.Sign() <= 0 {
			// CHADRON: Android does not care, re-encode to positive
			// return nil, errors.New("x509: RSA modulus is not a positive number")
			if needsReencode != nil {
				*needsReencode = true
			}
			p.N.Neg(p.N)
...
func parseCertificate(in *certificate) (*Certificate, error) {
	...
	out.PublicKey, err = parsePublicKey(out.PublicKeyAlgorithm, &in.TBSCertificate.PublicKey, &needsReencode)
	if err != nil {
		return nil, err
	}

	if needsReencode {
		out.RawSubjectPublicKeyInfo, err = MarshalPKIXPublicKey(out.PublicKey)
		if err != nil {
			return nil, err
		}
	}
...
func parseCertificateRequest(in *certificateRequest) (*CertificateRequest, error) {
	...
	var err error
	out.PublicKey, err = parsePublicKey(out.PublicKeyAlgorithm, &in.TBSCSR.PublicKey, nil)
	if err != nil {
		return nil, err
	}
